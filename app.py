...
app = FastAPI(title="LRA Holds Logger", description="Google Sheets integration for Little Revolution Artists", version="1.0.0")
                        ...
                        for i, cell in enumerate(all_data[2]):
                                    ...
                                    for date_str in dates:
                                                    for row_idx, row in enumerate(all_data[4:], start=5):
                                                                ...
                                                                return {"status": "success", "message": f"Logged {len(updates)} holds for {venue}", "dates_updated": len(updates)}
...
async def list_artists():
        return {"artists": list(ARTIST_SHEETS.keys()), "count": len(ARTIST_SHEETS)}

if __name__ == '__main__':
        port = int(os.environ.get('PORT', 5000))
    uvicorn.run("app:app", host="0.0.0.0", port=port, log_level="info")#!/usr/bin/env python3
import json
import os
from datetime import datetime
from typing import List, Dict
